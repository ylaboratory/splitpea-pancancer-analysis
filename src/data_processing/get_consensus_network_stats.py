...
out = open("/data/consensus/consensus_threshold.sizes.txt", 'w')
out.write('\t'.join(['cancer', 'direction', 'threshold', 'num_nodes', 'num_edges']) + '\n')
for c in folders_tuples:
    print('[' + time.strftime("%H:%M:%S", time.localtime()) + "] Loading " + c[0] + " consensus networks...")
    net_dir_ = directory_path + c[0] + '/' + c[1] + '/'

    c_consensus_neg = pickle.load(open(net_dir_ + "consensus_network_neg.pickle", 'rb'))
    c_consensus_pos = pickle.load(open(net_dir_ + "consensus_network_pos.pickle", 'rb'))  
    cneg_largest_cc = c_consensus_neg.copy()
    cpos_largest_cc = c_consensus_pos.copy() 

    out.write('\t'.join([c[0], 'negative', '0', str(len(cneg_largest_cc)), str(cneg_largest_cc.number_of_edges())]) + '\n')
    out.write('\t'.join([c[0], 'positive', '0', str(len(cpos_largest_cc)), str(cpos_largest_cc.number_of_edges())]) + '\n')
    ...
    for t in thresholds:
        ...
        with open("/data/consensus/consensus_thresh/" + c[0] + "_consensus_neg.thres_" + str(t) + ".pickle", 'wb') as out1:
            print(thres_neg)
            pickle.dump(thres_neg, out1)
        
        with open("/data/consensus/consensus_thresh/" + c[0] + "_consensus_pos.thres_" + str(t) + ".pickle", 'wb') as out2:
            print(thres_pos)
            pickle.dump(thres_pos, out2)

        with open("/data/consensus/consensus_thresh/" + c[0] + "_consensus_joint.thres_" + str(t) + ".pickle", 'wb') as out3:
            print(nx.compose(thres_neg, thres_pos))
            pickle.dump(nx.compose(thres_neg, thres_pos), out3) 

        out.write('\t'.join([c[0], 'negative', str(t), str(len(thres_neg)), str(thres_neg.number_of_edges())]) + '\n')
        out.write('\t'.join([c[0], 'positive', str(t), str(len(thres_pos)), str(thres_pos.number_of_edges())]) + '\n')